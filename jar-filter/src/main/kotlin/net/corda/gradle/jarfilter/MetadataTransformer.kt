...
internal class ClassMetadataTransformer(
    ...
    override fun visitConstructor(flags: Flags): KmConstructorVisitor? {
        ...
        return ConstructorFilter(logger, className, deletedConstructors, flags, cv)
    }
    ...
    override fun visitTypeAlias(flags: Flags, name: String): KmTypeAliasVisitor? {
        return if (deletedFields.any { it.name == name && it.extension == "()" }) {
            logger.info("-- removing typealias: {}", name)
            null
        } else {
            super.visitTypeAlias(flags, name)
        }
    }
}
...
private class ConstructorFilter(
     ...
     * that it was synthesised for, and which we DO expect to find.
     */
    private val deletedPrimary = deletedConstructors.map(MethodElement::asKotlinNonDefaultConstructor).firstOrNull()
    private var isDeleted: Boolean = false

    override fun visitExtensions(type: KmExtensionType): KmConstructorExtensionVisitor? {
        val cev = (super.visitExtensions(type) as? JvmConstructorExtensionVisitor) ?: return null
        return object : JvmConstructorExtensionVisitor(cev) {
            override fun visit(desc: JvmMethodSignature?) {
                val method = (desc ?: return).toMethodElement()
                if (deletedConstructors.contains(method)) {
                    if (IS_PRIMARY(flags)) {
                        logger.warn("Removing primary constructor: {}{}", className, method.descriptor)
                    } else {
                        logger.info("-- removing constructor: {}", method.signature)
                    }
                    isDeleted = true
                } else if (method == deletedPrimary) {
                    logger.info("-- removing default parameter values: {}", method.signature)
                }
            }
        }
    }
...
private class FunctionFilter(
    ...
    override fun visitExtensions(type: KmExtensionType): KmFunctionExtensionVisitor? {
        val fev = (super.visitExtensions(type) as? JvmFunctionExtensionVisitor) ?: return null
        return object : JvmFunctionExtensionVisitor(fev) {
            override fun visit(desc: JvmMethodSignature?) {
                val method = (desc ?: return).toMethodElement()
                if (deletedFunctions.contains(method)) {
                    logger.info("-- removing function: {}", method.signature)
                    isDeleted = true
                }
            }
        }
